...
def list_container_images(filter_name: Optional[str] = None) -> List[dict]:
 ...
 print(f"[WARNING] Error listing images: {result.stderr}")
 return []
 
 except Exception as e:
 print(f"[FAIL] Error listing container images: {e}")
 return []
...
def remove_container_image(image_name: str) -> bool:
 ...
 print(f"[WARNING] Failed to remove image {image_name}: {result.stderr}")
 return False
 
 except Exception as e:
 print(f"[FAIL] Error removing image {image_name}: {e}")
 return False
 ...
 *cmd,
 ...
 if success:
 logs.append(f"[PASS] Successfully built image: {tag}")
 else:
 logs.append(f"[FAIL] Failed to build image: {tag}")
 
 return success, logs
 
 except Exception as e:
 error_msg = f"[FAIL] Error building image {tag}: {str(e)}"
